...
scen.df <- data.frame(state=sort(rep(statesvec,4)),
                      mu_C = rep(rep(c(1,1,2,2),1),length(statesvec)),
                      mu_Lambda = rep(rep(c(1,2,1,2),1),length(statesvec)),
                      DeltaDelta = rep(c(rep(-0.5,4)),length(statesvec)),
...
pdf(file=file.path(figuredir,paste0("FigS10_Scenarios_Results-50_",datadatemax,".pdf")),height=4,width=6)
