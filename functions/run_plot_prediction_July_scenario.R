...
pdf(file=file.path(figuredir,"FigS5A_JulyPredictionResults.pdf"),height=6,width=8)
...
#  obspred.quant$training[obspred.quant$date <= as.Date("2020-06-20")] <- "Training"
  ...
  p95.cases.pred <- subset(obspred.quant,date <= as.Date("2020-07-22") & Output_Var=="Reported Cases")
  scenrow$p95.cases.pred <- sum(p95.cases.pred$p95)/nrow(p95.cases.pred)
  p95.deaths.pred <- subset(obspred.quant,date <= as.Date("2020-07-22") & Output_Var=="Confirmed Deaths")
  scenrow$p95.deaths.pred <- sum(p95.deaths.pred$p95)/nrow(p95.deaths.pred)
  ...
  plt<-ggplot(obspred.quant)+
    ...
    ggtitle(paste(scenrow$state,"\nPrediction Coverage:",
                  signif(scenrow$p95.deaths.pred,3),"(Deaths)",
                  signif(scenrow$p95.cases.pred,3),"(Cases)"))+
...
dev.off()
write.csv(scennew.df,file.path(figuredir,"FigS5A_JulyPredictionCoverage.csv"))
