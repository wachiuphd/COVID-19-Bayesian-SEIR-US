...
library(ggpubr)
datezero<-"2019-12-31"
datemax<-as.numeric(as.Date("2020-07-22")) -as.numeric(as.Date(datezero))
...
mobfit <- function(mdata.df,pdfname="",showplots=TRUE,
  ...
  for (statenow in unique(as.character(mfits.df$State.Abbr))) {
    for (varnow in unique(as.character(mfits.df$variable))) {
      ...
      rmax0<-((mean(data.table::last(dattmp1$value,5))/ys0)-thetamin1)/(1-thetamin1)
      ...
      if(is.character(try({
        if (is.na(parms0)[1]) {
          ...
          parmstmp <- subset(parms0,State.Abbr==statenow & MobilityMetric ==varnow)
          parmstmp.value <- parmstmp$value
          names(parmstmp.value) <- parmstmp$variable
          fittmp<-nlsLM(value~func(numDate,ys,thetamin,tautheta,ntheta,
                                   taus,taur,rmax),
                 data=dattmp,
                 start = list(ys = 10^parmstmp.value["log10ys"],
                              thetamin = parmstmp.value["thetamin"], 
                              tautheta = parmstmp.value["tautheta"],
                              ntheta = parmstmp.value["ntheta"],
                              taus = parmstmp.value["taus"],
                              taur = parmstmp.value["taur"],
                              rmax = parmstmp.value["rmax"]));
        }
...
## Load previous fits
mobparms0 <- fread("MobilityParms.csv")
```
...
ufits.df<- mobfit(udata.df,pdfname="Unacast-2020-07-21.pdf",parms0 = mobparms0)
```
...
gfits.df<-mobfit(gdata.df,pdfname="GoogleMobility-2020-07-21.pdf",parms0 = mobparms0)
```
...
ofits.df<-mobfit(odata.df,pdfname="OpenTable-2020-07-21.pdf",minmaxinit = TRUE,parms0 = mobparms0)
```
...
allfits.df$AbbrMobMet<-abbreviate(gsub("_","",
                                        ...
                                        minlength=10)
write.csv(allfits.df,file="MobilityFits-2020-07-21.csv",row.names=FALSE)
...
write.csv(fitparms.df,file="MobilityParms-2020-07-21.csv",row.names=FALSE)

pdf("Mobility-2020-07-21.pdf",height=8.5,width=11)
...
pdf("MobilityFitNorm-2020-07-21.pdf",height=6,width=9)
for (statenow in unique(as.character(allfits.df$State.Abbr))) {
  p<-ggplot(subset(allfits.df,State.Abbr==statenow))+
...
pdf("MobilityFitScale-2020-07-21.pdf",height=6,width=9)
for (statenow in unique(as.character(allfits.df$State.Abbr))) {
  p<-ggplot(subset(allfits.df,State.Abbr==statenow))+
...
print(fitsum)
write.csv(fitsum,file="MobilityParmsSummaryByState-2020-07-21.csv",row.names=FALSE)
```

```{r example figure}
allfits.df<-fread("MobilityFits-2020-07-21.csv")
...
extaulab.df <- data.frame(numDate=60+c(tautheta0/2,
                             tautheta0+taus0/2,
                             tautheta0+taus0+taur0/2),
                          numDate0=c(60,60+tautheta0,60+tautheta0+taus0),
                          numDate1=c(60+tautheta0,60+tautheta0+taus0,60+tautheta0+taus0+taur0),
                       Milestone=factor(c("Shelter","ReopenStart","ReopenMax"),
                                        levels=c("Shelter","ReopenStart","ReopenMax"))
                       )
extaulab.df$date <- as.Date(extaulab.df$numDate,origin=datezero)
extaulab.df$date0 <- as.Date(extaulab.df$numDate0,origin=datezero)
extaulab.df$date1 <- as.Date(extaulab.df$numDate1,origin=datezero)
extaulab.df$y <- 0.2

pex<-ggplot(exfit.df)+
  geom_line(aes(x=date,y=y),size=2,color="grey")+
  geom_point(data=extau.df,aes(x=date,y=y,color=Milestone))+
  geom_segment(data=extaulab.df,aes(x=date0,xend=date1,y=y,yend=y),arrow=arrow(ends="both",length=unit(0.1,"inches")))+
  geom_label(data=extaulab.df[1,],aes(x=date,y=0,
                                   label=deparse(bquote(tau[theta]))),
             parse=TRUE,
             hjust=0.5,vjust=-0.2)+
    geom_label(data=extaulab.df[2,],aes(x=date,y=0,
                                     label=deparse(bquote(tau[s]))),
             parse=TRUE,
             hjust=0.5,vjust=-0.2)+
    geom_label(data=extaulab.df[3,],aes(x=date,y=0,
                                     label=deparse(bquote(tau[r]))),
             parse=TRUE,
             hjust=0.5,vjust=-0.2)+
