...
TauR; # Days after TauS that max reopening occurs
...
GM_T50Testing = 83; # Time of 50% of final testing rate
GM_TauTesting = 7;
...
Dynamics { # ODEs
  ...
  ## Current Delta
  Deltatmp = (ctmp*beta/(c0*beta0) - pow(ThetaMin,1+HygienePwr))/
    (1 - pow(ThetaMin,1+HygienePwr));
  Deltatmp2 = (DeltaDelta > -1) ? (Deltatmp + DeltaDelta) : Deltatmp;
  Delta = (Deltatmp2 > 1) ? 1 : ((Deltatmp2 < 0) ? 0 : Deltatmp2);
  ReopenFit = ((Delta*(1-pow(ThetaMin,1+HygienePwr))-
  ...
  FTraced = (FTracedTmp > 1) ? 1 : FTracedTmp;
  ## Testing
  lambda1 = TestingTimeDep * lambda0;
  lambda = lambda1 * ((MuLambda>0) ? MuLambda : 1);
  lambda_C = TestingTimeDep * lambda0_C; 
  rho_C = TestingTimeDep * rho0_C;
  fracpos = FTraced1*lambda_C/(lambda_C + rho_C)+(1-FTraced1)*lambda1/(lambda1+rho); # fraction of infected that are tested and positive
  ## Case fatality
  ...
  delta = rho * CFR/(1-CFR);
  # rhotmp = (I_T > 0) ? (rho * (1 + I_U/I_T) + rho_C * I_C/I_T) : 0;
